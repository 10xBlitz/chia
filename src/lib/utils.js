 ...
 */
export function parseDateFromSupabase(range) {
    if (!range)
        return null;
...
export function calculateDiscountedPrice(originalPrice, discountPercentage) {
    ...
    if (typeof originalPrice !== "number" || originalPrice < 0) {
        console.error("Invalid input: Original price must be a non-negative number.");
    ...
    if (typeof discountPercentage !== "number" ||
        ...
        console.error("Invalid input: Discount percentage must be a number between 0 and 100.");
